...
class SocraticTeachingSystem:
    def __init__(self, api_key: str, base_url: str, model_name: str, debug_mode: bool = False,
    ...
    def add_to_advisor_history(self, evaluation: str, state: str, action: str) -> None:
        """添加顾问分析到历史记录"""
        ...
        for i in range(0, len(self.conversation_history) - 1, 2):
            if i + 1 < len(self.conversation_history):  # 确保有配对的教师回复
                ...
                if student_message["role"] == "student" and teacher_message["role"] == "teacher":
                    ...
                    advisor_index = i // 2
                    if advisor_index < len(self.advisor_history):
                        advisor_record = self.advisor_history[advisor_index]
                        formatted_history += f"[顾问分析]\n"
                        formatted_history += f"评估: {advisor_record['evaluation']}\n"
                        formatted_history += f"状态: {advisor_record['state']}\n"
                        formatted_history += f"行动: {advisor_record['action']}\n"

                        # 添加教学阶段轮数，但不添加总对话轮数
                        teaching_rounds = advisor_record.get("teaching_rounds", 0)
    ...
    def socratic_teaching_advisor(self, student_input: str) -> Dict[str, Any]:
...
"""
    ...
    def process_student_input(self, student_input: str) -> str:
        ...
        # 调用苏格拉底教学顾问
        advisor_result = self.socratic_teaching_advisor(student_input)

        # 获取状态和对应的提问操作
        previous_state = self.current_state
        state = advisor_result.get("state", self.current_state)
        action = self.get_action_for_state(state)
        evaluation = advisor_result.get("evaluation", "无法确定当前状态")
        ...
        if previous_state and state:
            ...
            if (prev_phase == 'b' and curr_phase == 'a') or \
                ...
                action = self.get_action_for_state(state)
                evaluation = f"防止阶段回退：保持在{state}状态而不是回退到{advisor_result['state']}"
        ...
        # 将顾问分析添加到历史记录中
        self.add_to_advisor_history(evaluation, state, action)
...
if __name__ == "__main__":
    ...
    teaching_system = SocraticTeachingSystem(
        ...
        max_teaching_rounds=MAX_TEACHING_ROUNDS  # 传入自定义的最大教学轮数
    )
